...
<div id='register-container' class="clearfix">
	...
	<?php /* <form id='register-form' method='post' class='clearfix' action='inc/register-processor.php'> */ ?>
		<div style='float:left; width:264px;'>
			<a href='admin.php' class='btn' style="margin:20px 0;">Admin Console</a>
			<div id='profile-pic'>
				<?php /*<div class='update-prof-pic btn'>UPDATE</div>*/ ?>
			</div>
			<div id='edit-buttons'>
				<?php /* <a class='btn' id='add-licence-button' style='display:none'>ADD ANOTHER LICENCE</a>
		...
		<?php for ($xo = 1; $xo <= 8; $xo++) { ?>
			<div id='page-<?php echo $xo; ?>' style='display:none' class='register-tab'>
				<?php if (file_exists("inc/register-form/uneditable/page" . $xo . ".php")) { require_once "inc/register-form/uneditable/page" . $xo . ".php"; } ?>
			</div>
		<?php } ?>
	<?php /* </form> */ ?>
...
#profile-pic {
	...
	float: left;
	margin-right:100px;
	background-image:url('jquery_upload_crop/upload_pic/<?php echo $getuser->details['profile-pic']; ?>');
...
$( '.form-element .element' ).each( function() {
	var content = $( this ).html();
	content = content.replace(/(<([^>]+)>)/ig,"");
	content = content.trim();
	content = content.replace("	"," ");
	content = content.replace("  "," ");
	content = '<input type="text" value="' + content + '" disabled="disabled">';
	$( this ).html( content );
});
</script>
<?php
